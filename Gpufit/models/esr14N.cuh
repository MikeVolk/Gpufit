#ifndef ESR14N_CUH_INCLUDED
#define ESR14N_CUH_INCLUDED
...
__device__ void calculate_esr3rt(
    ...
    std::size_t const user_info_size)
{
    ...
    // value
	// params [resp width c1 c2 c3 baseline]
	
	float const ahyp = 0.002158;
	float const arg1 = x-p[0]+ahyp;
	float const arg2 = x-p[0];
	float const arg3 = x-p[0]-ahyp;
	float const arg1sq = arg1*arg1;
	float const arg2sq = arg2*arg2;
	float const arg3sq = arg3*arg3;
	float const numall = p[1]*p[1];
	float const denom1 = 1/(arg1sq+numall);
	float const denom2 = 1/(arg2sq+numall);
	float const denom3 = 1/(arg3sq+numall);
	float const denom11 = 1/((arg1sq+numall)*(arg1sq+numall));
	float const denom22 = 1/((arg2sq+numall)*(arg2sq+numall));
	float const denom33 = 1/((arg3sq+numall)*(arg3sq+numall));
	
	
    value[point_index] = 1 + p[5] - p[2]*numall*denom1 - p[3]*numall*denom2 - p[4]*numall*denom3;

    // derivative
    float * current_derivative = derivative + point_index;

    current_derivative[0 * n_points]  = -2*p[2]*numall*arg1*denom11 - 2*p[3]*numall*arg2*denom22 - 2*p[4]*numall*arg3*denom33;
    current_derivative[1 * n_points]  = 2*numall*p[1]*p[2]*denom1*denom1-2*p[1]*p[2]*denom1+2*numall*p[1]*p[3]*denom2*denom2-2*p[1]*p[3]*denom2+2*numall*p[1]*p[4]*denom3*denom3-2*p[1]*p[4]*denom3;
	current_derivative[2 * n_points]  = -numall*denom1; 
    current_derivative[3 * n_points]  = -numall*denom2;
	current_derivative[4 * n_points]  = -numall*denom3;
