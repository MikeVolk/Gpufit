#ifndef ESRSINGLE_CUH_INCLUDED
#define ESRSINGLE_CUH_INCLUDED
...
__device__ void calculate_esrsingle(
    ...
    std::size_t const user_info_size)
{
    ...
    // value
	// params [resp width c  baseline]
	
	float const arg1 = x-p[0];
    ...
    value[point_index] = 1 + p[3] - p[2]*numall*denom1;

    // derivative
    float * current_derivative = derivative + point_index;

    current_derivative[0 * n_points]  = -2*p[2]*numall*arg1*denom11; 
    current_derivative[1 * n_points]  = 2*numall*p[1]*p[2]*denom1*denom1-2*p[1]*p[2]*denom1;
